...
def _include_file_list(filename: str) -> list:
  p = Path(filename)
  if not p.is_absolute():
    for dir in os.environ['HCONFIG_INCLUDE_PATH'].split(":"):
      filename_resolved = Path(dir) / p
      if filename_resolved.exists():
        p = filename_resolved
        break
  value = load_file(p)
  assert is_type(value, LIST_TYPES)
  return value


FUNCTIONS = {
  "H::int": _toint,
  "H::expandenv": _expandenv,
  "H::username": _username,
  "H::include_file_as_list": _include_file_list
}
...
def get_function(node: dict):
  return FUNCTIONS[list(node.keys())[0]]
...
def _merge_lists_by_dict_id(*lists, id_field="id"):
  ...
  # Do a first pass to process includes
  for item in all_items:
    if is_function(item) and get_function(item) == _include_file_list:
      all_items.extend(evaluate_function(item))
  all_items = [item for item in all_items if not (is_function(item) and get_function(item) == _include_file_list)]
  ...
  with atomic_output_file(target_filename, make_parents=make_parents) as temp_out:
    with open(temp_out, "w") as out:
      ...
      if target_path.suffix == '.json':
        output_format = 'json'
      elif target_path.suffix == '.yml' or target_path.suffix == '.yaml':
        output_format = 'yaml'
